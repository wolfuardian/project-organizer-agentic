...
```bash
# 安裝依賴（Python 3.12+，需要 PySide6）
python3 -m pip install PySide6

# 執行應用程式
python3 main.py

# 執行測試（不需要 pytest，使用 unittest）
python3 -m pytest tests/          # 若有 pytest
python3 -m unittest discover tests # 或用內建 unittest
```

唯一外部依賴為 **PySide6**，目前無 `requirements.txt`。
...
根目錄保留 16 個 shim 檔案，每個僅 1–2 行 `from xxx import *`，轉發至 DDD 新路徑：
`backup.py` `batch_rename.py` `classifier.py` `database.py` `duplicate_finder.py` `file_ops.py` `fuzzy.py` `git_utils.py` `main_window.py` `report_exporter.py` `rule_engine.py` `scanner.py` `session_manager.py` `templates.py` `themes.py` `tree_model.py`

另有根目錄 `test_use_cases.py`（與 `tests/test_use_cases.py` 完全相同的副本）和 `run.bat`（Windows 啟動腳本）。
...
- 語言：程式碼（變數、函式、類別）用英文命名；UI 字串與註解用繁體中文
- commit 格式：`feat(phaseN): 功能描述` 或 `fix/refactor/perf: 描述`
- domain 層禁止 import PySide6 或 sqlite3；infrastructure 禁止 import PySide6
- Repository 方法回傳 `domain/models.py` 的 dataclass，不回傳 sqlite3.Row
- 新增 Repository 時須在 `domain/protocols.py` 定義對應 Protocol

## 注意事項

- **勿刪除根目錄 shim 檔案**：它們確保 `main.py` 中的舊式 import 正常運作，移除會導致 ImportError
- **SQLite 伴隨檔**：WAL 模式會產生 `.db-wal` 和 `.db-shm`，已列入 `.gitignore`，正常現象
- **資料目錄**：`~/.project-organizer/` 存放 `data.db` + `trash/` + `backups/`，首次執行自動建立
- **`__init__.py`**：各層皆有 `__init__.py`，部分用於 re-export 以支援 shim 層
- **掃描效能**：大目錄掃描以批次寫入 DB（每 50 筆 commit 一次），避免 UI 阻塞
